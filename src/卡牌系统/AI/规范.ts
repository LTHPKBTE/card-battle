...
操作 operations / cost 每一项至少含 type (下面写「数值」的地方都能用上面的公式):
  ...
  ATTACK        { "target": 目标, "pierce": 可选布尔 }  (让来源卡攻击, 每回合 1 次)
...
必须遵守的规则:
  ...
  5. 护盾是会被打空的: 打空后要再恢复必须显式写 RESTORE_SHIELD 或给 shield 加修正,
     卡自己不会自动回盾; 想写「回盾」类效果就用 RESTORE_SHIELD
  6. 键名、枚举值必须与上面完全一致 (大小写敏感), 不得发明新字段
  7. 效果里的循环/变量是有上限的 (见上), 不要用它们做真正的程序 —— 只用来表达卡面那几十个字
  8. 语法表达不了的效果: 直接省略, 不要硬凑
  9. 「是否发动由玩家/角色自己决定」的效果才写 ask: true (加上 ask_default: "SKIP" 更稳),
